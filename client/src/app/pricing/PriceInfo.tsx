export default function PriceInfo({
  ...
  content: string;
  benifits: string;
  more_benifits?: string;
  most_benifits?: string;
}) {
  return (
    <div className="border-2 rounded-lg w-72 h-full mx-auto flex flex-col justify-around items-center gap-y-6 py-5">
      <h1 className="font-semibold text-3xl">{type}</h1>
      <p className="text-[#4B5563] text-xs">{content}</p>
      <h1 className="font-medium text-4xl">₹ {cost}</h1>
      <p className="text-[#4B5563] text-xs">Pay once, Use forever</p>
      ...
      <button onClick={pay} className="bg-blue-600 w-4/5 text-white">
        PAY
      </button>
    </div>
  );
